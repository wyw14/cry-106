...
func (s *Service) start(groupID, purpose, target string) (operation.Owner, error) {
	...
	if err := s.arbiter.Begin(groupID, purpose); err != nil {
		return operation.Owner{}, err
	}
	owner, err := s.owners.Acquire("fan-group:"+groupID, operationID, purpose)
	if err != nil {
		s.arbiter.Finish(groupID, purpose, "rejected")
		return operation.Owner{}, err
	}
